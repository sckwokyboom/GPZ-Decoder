package model.binarycanonicalconverter.service

import model.binarycanonicalconverter.BinaryMatrix
import mu.KotlinLogging

object BinaryMatrixConverter {
    private val logger = KotlinLogging.logger {}

    fun canonical(matrix: BinaryMatrix): BinaryMatrix {
        ...
        val rearrangedMatrix = BinaryMatrix(matrix.rows, matrix.cols)
        ...
        val eyeMatrix = BinaryMatrix.eye(aTransposed.rows)
        val generativeMatrix = BinaryMatrix.hstack(aTransposed, eyeMatrix)
        logger.trace { "порождающая матрица для проверочной с примененной перестановкой\n$generativeMatrix" }

        // Восстановление исходной матрицы G
        val restoredG = BinaryMatrix(generativeMatrix.rows, generativeMatrix.cols)
